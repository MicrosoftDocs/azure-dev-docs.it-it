...
**Gruppo di risorse**: i gruppi di risorse offrono un modo per organizzare le risorse di Azure in gruppi per la gestione. Le risorse create in Azure verranno archiviate in un gruppo di risorse, operazione simile al salvataggio di un file in una cartella nel computer.

**Hosting**: per essere eseguito in Azure, il codice deve essere ospitato in un servizio in grado di supportare l'esecuzione di codice fornito dall'utente.

**Servizi gestiti**: Azure offre alcuni servizi in cui l'utente fornisce dati o informazioni ad Azure e l'implementazione di Azure esegue l'azione appropriata. Un esempio è Archiviazione BLOB di Azure, in cui si forniscono file e Azure gestisce la lettura, la scrittura e la persistenza.
